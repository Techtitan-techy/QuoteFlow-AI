...
export function QuotationEditor({
  ...
  const { data: clients = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("clients")
        .select("id, business_name, contact_person, email, phone, billing_address, gst_number")
        .order("business_name");
      return data ?? [];
    },
  });
  ...
  const totals = useMemo(
    () => calcTotal(q.subtotal, q.discount_percent, q.tax_percent),
    [q.subtotal, q.discount_percent, q.tax_percent],
  );
  useEffect(() => {
    setQ((s) => ({ ...s, total: totals.total }));
  }, [totals.total]);
  ...
  const toggleFeature = (label: string) => {
    setQ((s) => {
      ...
      if (idx === -1)
        return { ...s, features: [...s.features, { id: uid(), label, included: true }] };
  ...
  const addCustomFeature = () => {
    ...
    setQ((s) => ({
      ...s,
      features: [...s.features, { id: uid(), label: addFeatureLabel.trim(), included: true }],
    }));
    setAddFeatureLabel("");
  };
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <Tabs defaultValue="details">
        ...
        <TabsContent value="details" className="mt-4 space-y-4">
          <Card className="p-6">
            <h2 className="mb-4 flex items-center gap-2 font-semibold">
              <Users className="h-4 w-4 text-primary" /> Basics
            </h2>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <F label="Client">
                <Select
                  value={q.client_id ?? "none"}
                  onValueChange={(v) => setQ({ ...q, client_id: v === "none" ? null : v })}
                >
                  <SelectTrigger>
              ...
              <F label="Quotation Number">
                <Input
                  value={q.quotation_number}
                  onChange={(e) => setQ({ ...q, quotation_number: e.target.value })}
                />
              </F>
              <F label="Currency">
              ...
              <F label="Issue Date">
                <Input
                  type="date"
                  value={q.issue_date}
                  onChange={(e) => setQ({ ...q, issue_date: e.target.value })}
                />
              </F>
              <F label="Expiry Date">
                <Input
                  type="date"
                  value={q.expiry_date}
                  onChange={(e) => setQ({ ...q, expiry_date: e.target.value })}
                />
              </F>
            </div>
          </Card>
          ...
          <Card className="p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                {q.features.filter((f) => f.included).length} selected
              </span>
            </div>
            ...
            <div className="space-y-6">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {items.map((label) => {
                      ...
                      return (
                        <label
                          key={label}
                          className="flex cursor-pointer items-center gap-2 rounded-md border p-2 text-sm transition-colors hover:bg-accent/40"
                        >
                          <Checkbox
                            checked={checked}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Input
                    value={addFeatureLabel}
                    onChange={(e) => setAddFeatureLabel(e.target.value)}
                  ...
                  />
                  <Button type="button" onClick={addCustomFeature} className="gap-1">
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setQ((s) => ({
                              ...s,
                              features: s.features.filter((x) => x.id !== f.id),
                            }))
                          }
                        >
                          <X className="h-3 w-3" />
        ...
        <TabsContent value="pricing" className="mt-4 space-y-4">
          <Card className="p-6">
            <h2 className="mb-4 flex items-center gap-2 font-semibold">
              <Wallet className="h-4 w-4 text-primary" /> Pricing
            </h2>
            <div className="grid gap-4 sm:grid-cols-4">
              <F label={`Final Project Price (${q.currency})`}>
                <Input
                  ...
                  step="0.01"
                  value={q.subtotal}
                  onChange={(e) => setQ({ ...q, subtotal: Number(e.target.value) })}
                />
              </F>
              <F label="Discount %">
                <Input
                  ...
                  onChange={(e) => setQ({ ...q, discount_percent: Number(e.target.value) })}
                />
              </F>
              <F label="Tax %">
                <Input
                  ...
                  step="0.01"
                  value={q.tax_percent}
                  onChange={(e) => setQ({ ...q, tax_percent: Number(e.target.value) })}
                />
              </F>
          ...
          <Card className="p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span
                className={`text-xs ${q.milestones.reduce((s, m) => s + m.percent, 0) === 100 ? "text-success" : "text-warning"}`}
              >
            ...
            <div className="space-y-2">
              {q.milestones.map((m, i) => (
                <div
                  key={m.id}
                  className="grid grid-cols-[1fr_100px_140px_auto] items-center gap-2"
                >
                  <Input
                    value={m.label}
                    onChange={(e) =>
                      setQ({
                        ...q,
                        milestones: q.milestones.map((x, j) =>
                  ...
                  <div className="text-sm font-semibold text-right">
                    {formatMoney((q.subtotal * m.percent) / 100, q.currency)}
                  </div>
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() =>
                      setQ({ ...q, milestones: q.milestones.filter((_, j) => j !== i) })
                    }
                  >
              ...
              <Button
                ...
                onClick={() =>
                  setQ({
                    ...
                    milestones: [
                      ...q.milestones,
                      { id: uid(), label: "New milestone", percent: 0 },
                    ],
                  })
                }
              >
                ...
                <div key={t.id} className="grid grid-cols-[120px_1fr_auto] items-center gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setQ({
                        ...q,
                        timeline: q.timeline.map((x, j) =>
                          j === i ? { ...x, week: e.target.value } : x,
                        ),
                      })
                    }
                  />
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() => setQ({ ...q, timeline: q.timeline.filter((_, j) => j !== i) })}
                  >
                    <Trash2 className="h-4 w-4 text-destructive" />
                  </Button>
                </div>
              ...
              <Button
                ...
                onClick={() =>
                  setQ({
                    ...
                    timeline: [
                      ...q.timeline,
                      { id: uid(), week: `Week ${q.timeline.length + 1}`, label: "" },
                    ],
                  })
                }
              >
        ...
        <TabsContent value="notes" className="mt-4 space-y-4">
          ...
          <Card className="p-6">
            ...
            <Textarea
              rows={6}
              value={q.terms}
              onChange={(e) => setQ({ ...q, terms: e.target.value })}
      ...
      <AIAssistDialog
        ...
        onApply={(result) => {
          setQ((s) => ({
            ...s,
            title:
              s.title || (result.summary ? result.summary.split(".")[0].slice(0, 80) : s.title),
            ...
            milestones: result.milestones.length
              ? result.milestones.map((m) => ({ id: uid(), ...m }))
              : s.milestones,
            timeline: result.timeline.length
              ? result.timeline.map((t) => ({ id: uid(), ...t }))
              : s.timeline,
          }));
...
function AIAssistDialog({
  ...
  onApply: (r: Awaited<ReturnType<typeof analyzeDocumentation>>) => void;
}) {
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Wand2 className="h-5 w-5 text-primary" /> AI Assist — Analyze Documentation
          </DialogTitle>
        </DialogHeader>
        <p className="text-sm text-muted-foreground">
          Paste project documentation, a spec, or a brief. AI will extract features, scope,
          timeline, milestones, and suggest pricing.
        </p>
        <div className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Paste project docs, requirements, or brief here…"
            />
          </div>
        </div>
        <DialogFooter>
